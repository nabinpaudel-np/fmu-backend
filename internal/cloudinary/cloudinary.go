...
const uploadResourceTypeImage = "image"
...
func (c *Client) SignUpload(folder, resourceType string) (SignPayload, error) {
	...
	if resourceType != uploadResourceTypeImage {
		return SignPayload{}, fmt.Errorf("%w: resource_type must be %q", ErrNotConfigured, uploadResourceTypeImage)
	}
	ts := c.now().Unix()
