...
type ProgramService interface {
	...
	Update(ctx context.Context, id string, req *UpdateProgramRequest) (*ProgramResponse, error)
	...
	ListAllFull(ctx context.Context) ([]ProgramResponse, error)
}
...
func (s *programService) Update(ctx context.Context, id string, req *UpdateProgramRequest) (*ProgramResponse, error) {
	row, err := s.repo.Update(ctx, toUpdateParams(id, req))
	if err != nil {
		log.Default().Printf("failed to update program %s: %v", id, err)
		return nil, err
	}
	return toResponse(row), nil
}
...
func (s *programService) ListAllFull(ctx context.Context) ([]ProgramResponse, error) {
	rows, err := s.repo.ListAll(ctx)
	if err != nil {
		log.Default().Printf("failed to list all programs: %v", err)
		return nil, err
	}
	items := make([]ProgramResponse, len(rows))
	for i, row := range rows {
