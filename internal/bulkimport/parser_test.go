...
func TestParse_Happy(t *testing.T) {
	...
	if got := res.Rows[0].Slug; got != "mit" {
		t.Errorf("row 1 slug: got %q, want %q", got, "mit")
	}
	if got, want := res.Rows[0].TuitionMin, int32(20000); got != want {
		t.Errorf("row 1 tuition_min: got %d, want %d", got, want)
	}
...
func TestParse_BadNumeric(t *testing.T) {
	res := Parse(read(t, "bad_numeric.csv"), StatusPublished)

	// The parser surfaces the error AND still decodes the rows that didn't
	// error. All-or-nothing happens at the handler/service layer: the
	// handler refuses the upload entirely if res.Errors is non-empty.
	if len(res.Errors) == 0 {
	...
	for _, e := range res.Errors {
		if e.Row == 2 && e.Column == "tuition_min" {
			found = true
		}
	}
	if !found {
		t.Errorf("expected error on row 2 tuition_min, got %+v", res.Errors)
	}
}
...
func TestParse_BOM(t *testing.T) {
	...
	if res.Rows[0].Slug != "bom-uni" {
		t.Errorf("expected slug=bom-uni, got %q", res.Rows[0].Slug)
	}
}
...
func TestParse_MissingRequired(t *testing.T) {
	...
	// Row is missing contact_email, website, and overview — all required
	// when status=published. The handler refuses the upload if any errors
	// surface; the parser still reports each missing field for the
	// admin's benefit.
	if len(res.Errors) == 0 {
		t.Fatal("expected missing-required errors, got none")
	}
	cols := map[string]bool{}
	for _, e := range res.Errors {
		cols[e.Column] = true
	}
	for _, want := range []string{"contact_email", "website", "overview"} {
		if !cols[want] {
			t.Errorf("expected error on column %q, got %+v", want, res.Errors)
		}
	}
}
